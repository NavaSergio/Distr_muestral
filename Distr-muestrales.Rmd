...
Distribucion <- base +
  geom_function(aes(colour = "normal"), fun = pnorm) +
  geom_function(aes(colour = "t, df =  1"), fun = pt, args = list(df = 1)) +
  geom_function(aes(colour = "t, df =  3"), fun = pt, args = list(df = 3)) +
  geom_function(aes(colour = "t, df =  9"), fun = pt, args = list(df = 9)) +
  geom_function(aes(colour = "t, df = 27"), fun = pt, args = list(df = 27)) +
  theme_classic() + 
  labs(y = "probabilidad",
       title = "Distribución t y normal estándar",
       x = "x")
...
set.seed(19)
matrix(sample(datos),nrow = 5) %>% kable(align = "ccccc") %>% 
  kable_styling(font_size = 9) 
```
...
k <- 1:length(datos)
pk <- (k -.5)/length(datos)
tabla <- data.frame(k,x=datos,Pk=pk,Porcentaje=pk*100)
tabla %>% kable(align = "cccc",digits=2) %>% 
  kable_styling(font_size = 7) 
...
![](figuras/probpaperLandscape2.png){width=200%}
...
|        |       |
|:-------|------:|
| ![alt text](figuras/Papel1.png){ width=45% } | ![alt text](figuras/Papel2.png){ width=45% }  | 
...
|        |       |
|-------:|:------|
| ![alt text](figuras/Papel3.png){ width=45% } | ![alt text](figuras/Papel4.png){ width=45% }  | 
| ![alt text](figuras/Papel5.png){ width=45% } | $P\left[ \mu -\sigma <X<\mu +\sigma  \right] =  0.683 \newline P\left[ \mu -2\sigma <X<\mu +2\sigma  \right] = 0.954 \newline P\left[ \mu -3\sigma <X<\mu +3\sigma  \right] = 0.997$ | 
...
```{r}
tiempofalla <- c(176.1, 76.6, 150.4, 197.6, 35.3, 24.7, 55.0, 73.0, 
                 124.5, 155.7, 34.9, 122.8, 90.6, 2.4, 46.0, 133.8,
                 99.6, 131.5, 40.4, 40.4)
matrix(tiempofalla,ncol = 4) %>% kable(align = "cccc") %>% 
  kable_styling(font_size = 9) 
```
...
Ver la siguiente aplicación en Shiny [qqnorm](https://s3rgionava.shinyapps.io/QQnorm/?_ga=2.74771473.2040383487.1666298329-1276638344.1663958366)

![Pantalla de Shiny app QQnorm](figuras/QQnorm.png){width=70%}
...
- Ejecute el archivo **distr_muestral_pequena2.R** en  [https://rstudio.cloud/content/4731243](https://rstudio.cloud/content/4731243) y discuta las salidas.
...
entonces $X \sim F_{n_1,n_2}$ a veces denotada como $F(n_{1},n_{2})$ y su función de densidad está dada por 
$$
...
$$
para $n_{1} = 1,2,\ldots$ y $n_{2} = 1,2,\ldots \,$.
La distribución $F$ se usa para inferencia estadística de razones de varianzas de dos poblaciones normales.
También se usa para inferencia estadística de razones de tasas de dos poblaciones exponenciales.



##

```{r distF, fig.height=6}
base <-
  ggplot() +
  xlim(0, 5)

Densidad <-base +
  geom_function(aes(colour = "F, n1 = 1, n2 = 1"), fun = df, args = list(df1 = 1,df2 = 1)) +
  geom_function(aes(colour = "F, n1 = 10, n2 = 1"), fun = df, args = list(df1 = 10,df2 = 1)) +
  geom_function(aes(colour = "F, n1 = 1, n2 = 10"), fun = df, args = list(df1 = 1,df2 = 10)) +
  geom_function(aes(colour = "F, n1 = 75, n2 = 75"), fun = df, args = list(df1 = 75,df2 = 75)) +
  theme_classic() + 
  labs(y = "probabilidad",
       title = "Densidad F",
       x = "x")
...
Distribucion <-base +
  geom_function(aes(colour = "F, n1 = 1, n2 = 1"), fun = pf, args = list(df1 = 1,df2 = 1)) +
  geom_function(aes(colour = "F, n1 = 10, n2 = 1"), fun = pf, args = list(df1 = 10,df2 = 1)) +
  geom_function(aes(colour = "F, n1 = 1, n2 = 10"), fun = pf, args = list(df1 = 1,df2 = 10)) +
  geom_function(aes(colour = "F, n1 = 75, n2 = 75"), fun = pf, args = list(df1 = 75,df2 = 75)) +
  theme_classic() + 
  labs(y = "probabilidad",
